import filecmp
import subprocess
import sys

import numpy as np
import pytest
import sarkit.cphd as skcphd
from PIL import Image

import tests.utils
from sarkit_assurance import names


def get_chan_ids(cphd_path):
    with cphd_path.open("rb") as f, skcphd.Reader(f) as r:
        all_ch_ids = [
            x.text
            for x in r.metadata.xmltree.findall("{*}Data/{*}Channel/{*}Identifier")
        ]
        ref_ch_id = r.metadata.xmltree.findtext("{*}Channel/{*}RefChId")
        return all_ch_ids, ref_ch_id


def test_nominal(tmp_path, example_cphd):
    out_thumb = tmp_path / "out.png"
    expected_max_num_pixels = 2**10
    subprocess.check_call(
        [
            sys.executable,
            "-m",
            "sarkit_assurance.cphd_thumb",
            str(example_cphd),
            out_thumb,
            "--num-mebipixels",
            str(expected_max_num_pixels / 2**20),
        ],
    )
    img = Image.open(out_thumb)
    assert img.format == "PNG"
    assert img.mode == "L"  # 8 bit grayscale
    assert 0 < np.prod(img.size) <= expected_max_num_pixels


def test_bad_channel_id(tmp_path, multichan_cphd):
    bad_channel = "NOTAVALIDCHANNELID"
    ch_ids, _ = get_chan_ids(multichan_cphd)
    assert bad_channel not in set(ch_ids)
    with pytest.raises(subprocess.CalledProcessError):
        subprocess.check_call(
            [
                sys.executable,
                "-m",
                "sarkit_assurance.cphd_thumb",
                str(multichan_cphd),
                str(tmp_path / "{ch_id}.png"),
                f"--channel-id={ch_ids[0]}",
                f"--channel-id={bad_channel}",
            ],
        )
    assert len(list(tmp_path.iterdir())) == 0


def test_smart_open(tmp_path, example_cphd):
    with tests.utils.static_http_server(example_cphd.parent) as server_url:
        out_thumb = tmp_path / "out.png"
        expected_max_num_pixels = 2**10
        subprocess.check_call(
            [
                sys.executable,
                "-m",
                "sarkit_assurance.cphd_thumb",
                f"{server_url}/{example_cphd.name}",
                str(out_thumb),
                "--num-mebipixels",
                str(expected_max_num_pixels / 2**20),
            ],
        )
        img = Image.open(out_thumb)
        assert img.format == "PNG"
        assert img.mode == "L"  # 8 bit grayscale
        assert 0 < np.prod(img.size) <= expected_max_num_pixels


def test_multichan(tmp_path, multichan_cphd):
    ch_ids, ref_chan = get_chan_ids(multichan_cphd)

    # don't specify ch_ids
    subprocess.check_call(
        [
            sys.executable,
            "-m",
            "sarkit_assurance.cphd_thumb",
            str(multichan_cphd),
            str(tmp_path / "{ch_id}_auto.png"),
        ],
    )
    actual_files = set(x.name for x in tmp_path.iterdir())
    assert len(actual_files) == len(ch_ids)

    # specify ch_ids
    nonref_chan = (set(ch_ids) - {ref_chan}).pop()
    subprocess.check_call(
        [
            sys.executable,
            "-m",
            "sarkit_assurance.cphd_thumb",
            str(multichan_cphd),
            "--chan",
            nonref_chan,
            "--ref-chan",
            str(tmp_path / "{ch_id}_manual.png"),
        ],
    )
    assert len(list(tmp_path.iterdir())) == len(ch_ids) + 2
    assert filecmp.cmp(
        (tmp_path / f"{names.sanitize_name(ref_chan)}_auto.png"),
        (tmp_path / f"{names.sanitize_name(ref_chan)}_manual.png"),
        shallow=False,
    )
    assert filecmp.cmp(
        (tmp_path / f"{names.sanitize_name(nonref_chan)}_auto.png"),
        (tmp_path / f"{names.sanitize_name(nonref_chan)}_manual.png"),
        shallow=False,
    )


def test_multichan_clobber(tmp_path, multichan_cphd):
    with pytest.raises(subprocess.CalledProcessError):
        subprocess.check_call(
            [
                sys.executable,
                "-m",
                "sarkit_assurance.cphd_thumb",
                str(multichan_cphd),
                str(tmp_path / "missing_chid.png"),
            ],
        )
